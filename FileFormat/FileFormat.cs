...
namespace RFDI.FileFormat
{
    ...
    {
        public uint dataCount;	    //光谱点数量
        public double firstX;		//开始波数
        public double lastX;		//结束波数
        public double maxYValue;	//最大Y值
        public double minYValue;	//最小Y值
        public double scaleYValue;	//放大系数
        public DateTime time;       //扫描时间
        public string resolution;   //分辨率
        public string specType;   //光谱类型(近红外，拉曼...)
        public string xType;     //X轴坐标类型
        public string dataType;  //数据类型(吸收谱, 干涉图...)
    };

    public enum EnumFileType {FDI, OPUS, SPC, JCAMP, DPT, UNKNOWN }       //FDI格式与OPUS格式相同
    public class SpecFileFormat
    {
                ...
                switch (fileType)
                {
                    case EnumFileType.FDI:
                        if (OpenedFileType != EnumFileType.FDI)
                            throw new Exception("源文件不是没有包含检测信息，不能保存为FDI格式");
        ...
        public IntPtr ReadIdentBlock(string filename)
        {
            ...
            switch (type)
            {
                ...
                case EnumFileType.FDI:
                    ...
                    break;
                case EnumFileType.SPC:
                    retPtr = SPCFile.ReadIdentifyBlock(filename);
                    ErrorString = SPCFile.ErrorString;
                    break;
                default:
                    ErrorString = "文件类型参数错误";
...
//                 return EnumFileType.JCAMP;
    ...
    public static class OPUSFileFormat
    {
        ...
        static OpusHeader IsOpusFile(MappedFile mapfile)
        {
        ...
        public static bool IsOpusFile(string filename)
        {
            ...
            OpusHeader header = IsOpusFile(mapfile);
            mapfile.UnloadFile();
        ...
        static float GetResolution(MappedFile mapFile)
        {
            ...
            uint i;
            DirectoryBlock blockEntry = mapFile.MapStruct<DirectoryBlock>(header.pFirstDirectory, typeof(DirectoryBlock));
            for (i = 0; i < header.curDirBlkCount; i++)	//在block Entries中查找与dataCount相同block
            {
                ...
                //目前只检测：AB数据 和 数据状态参数
                if (((blockEntry.type >> SHIFT_DBSCPLX) & MASK_DBMCPLX) == 0 &&		        //振幅数据 
                    ((blockEntry.type >> SHIFT_DBSPARM) & MASK_DBMPARM) == DBTAQPAR)		//扫描设置参数
                    break;
            }
        ...
        public static FileParameter ReadOpusFileParameter(string filename, uint blockType)
        {
            ...
            try
            {
                ...
                {
                    //目前只检测：AB数据 和 数据状态参数
                    if (((blockEntry.type >> SHIFT_DBSCPLX) & MASK_DBMCPLX) == DBTAMPL &&		//振幅数据 
                        ((blockEntry.type >> SHIFT_DBSPARM) & MASK_DBMPARM) == DBTDSTAT &&		//数据状态参数
                        ((blockEntry.type >> SHIFT_DBSDATA) & MASK_DBMDATA) == blockType)	//DBTAB: AB数据 DBTSPEC:RSC数据
                        break;
        ...
        public static float[] ReadOpusFileData(string filename, uint blockType)
        {
            ...
            try
            {
                ...
                for (i = 0; i < header.curDirBlkCount; i++)	//在block Entries中查找与dataCount相同block
                {
                    //目前只检测：AB数据 和 数据状态参数
                    if (((blockEntry.type >> SHIFT_DBSCPLX) & MASK_DBMCPLX) == DBTAMPL &&		//振幅数据 
                        ((blockEntry.type >> SHIFT_DBSPARM) & MASK_DBMPARM) == 0 &&			    //不是参数,而是数据
                        ((blockEntry.type >> SHIFT_DBSDATA) & MASK_DBMDATA) == blockType)	//DBTAB: AB数据 DBTSPEC:RSC数据
                        break;
        ...
        public static bool WriteIdentBlock(string filename, IntPtr writeData, uint dataSize)
        {
            ...
            try
            {
                ...
                //是否是OPUS文件
                OpusHeader header = IsOpusFile(fileMap);
                if (header.curDirBlkCount == 0)     //不是OPUS格式
                    throw new Exception("不是OPUS光谱");
        ...
        public static bool DeleteIdentBlock(string filename)
        {
            ...
            try
            {
                ...
                {
                    identifyOffset = (int)readStream.BaseStream.Seek(0, SeekOrigin.Current);
                    if(readStream.ReadUInt32() == IDENTFYYTYPE)
                        break;
                }

                if(identifyOffset == 0)     //没找到IdentifyBlock
                ...
                writeStream.Write(IDENTFYYTYPE - 11);   //改变类型，相当于删除IdentifyBlock
    ...
    public unsafe static class SPCFile
    {
        ...
        /// <summary>
        /// 读取各种格式的数据，并转换到float数组, 只能是4Byte数据
        /// </summary>
        ...
        public static bool HasIdentifyBlock(string filename)
        {
            MappedFile mapFile = null;
            ...
            try
            {
                ...
                //检查是不是SampleResult数据
                //紧接SPCLogHeader后读取标志, 看看是不是 RFDAMark = 0x52464441(RFDI的二进制码)
                int[] tempint = mapFile.ReadIntData(header.flogoff + logHeaderSize, 1);
                if (tempint.Length != 1 || (uint)tempint[0] != SpecFileFormat.RFDAMark)
                    throw new Exception("没有找到检测结果");
        ...
        public static IntPtr ReadIdentifyBlock(string filename)
        {
            MappedFile mapFile = null;
